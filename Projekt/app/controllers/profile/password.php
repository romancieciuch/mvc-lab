<?php
	...
	if (!empty($_POST["form-sent"]) && !empty($grecaptcha)) {
		$dto = App\Models\DTO\ChangePasswordDTO::parse($_DB->sanitize_array($_POST) + ["id" => $user->id]);
		$errors = $dto->errors;
	}

	// Czy wszystko OK
	if (!empty($_POST["form-sent"]) && !empty($grecaptcha) && empty($dto->errors)) {
		$userdata = $_USER->change_password($dto);
		$errors = $userdata["errors"] ?? [];

		if (empty($errors)) {
			$user = App\Models\DTO\UserDTO::parse($_USER->me($user));
			$message["global"] = "Hasło zostało zmienione";
		}
	}