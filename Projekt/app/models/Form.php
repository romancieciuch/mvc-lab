...
class Form {
	...
	public function global_info (array $args = []) : string {
		$title = !empty($args["title"]) ? '<span class="alert-info-title">' . $args["title"] . '</span>' : '';
		$desc = !empty($args["desc"]) ? '<p class="alert-info-text">' . $args["desc"] . '</p>' : '';

		if (empty($title) && empty($desc)) return '';

		return '
			<div class="alert-info" role="status">
				<svg class="alert-info-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
					<path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" />
				</svg>
				<div class="alert-info-content">
					<div class="alert-info-title">' . $title . '</div>
					<p class="alert-info-text">' . $desc . '</p>
				</div>
			</div>
		';
	}
